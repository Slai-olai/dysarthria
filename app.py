...
# ──────────────────────────────────────────────────────────
# LOAD MODEL (cached supaya tidak reload setiap interaksi)
# ──────────────────────────────────────────────────────────
@st.cache_resource
def load_model():
...
if uploaded_file is not None:
    ...
    with st.spinner("Memproses audio dan menjalankan model..."):
        try:
            mel_db, y_audio, sr = audio_to_melspectrogram(uploaded_file)
    ...
    ax1.set_title("Mel Spectrogram dari Audio yang Diupload")
    st.pyplot(fig1)
